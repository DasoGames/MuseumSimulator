...
public class ChickenDisplay : MonoBehaviour, IInteractable
{
    ...
    [Header("결과물 데이터")]
    // 💡 진열대에서 빈손으로 꺼낼 때 손님에게 판매할 '테이크아웃 치킨(닭강정)' 구조체 데이터
    public HoldableObject chickenCupData; // 이름: "치킨컵" (또는 "닭강정컵")
    ...
    public void Interact()
    {
        ...
        if (player.IsHoldingItem)
        {
            ...
            else if (heldData.objectName == "탄치킨")
            {
        ...
        if (!player.IsHoldingItem)
        {
            if (currentStock > 0)
            {
                ...
                player.HoldNewData(chickenCupData); // 플레이어 손에 테이크아웃 컵 쥐여주기
